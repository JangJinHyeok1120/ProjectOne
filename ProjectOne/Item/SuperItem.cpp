...
#include "ProjectOne/AI/AICharacter.h"
...
{
	static ConstructorHelpers::FObjectFinder<UStaticMesh> MESH(TEXT("StaticMesh'/Game/BuildMap/Object/Item_jar/SM_Item_jar.SM_Item_jar'"));
	if (MESH.Succeeded())
...
void ASuperItem::OnCollisionOverlap(UPrimitiveComponent * OverlappedComp, AActor * OtherActor, UPrimitiveComponent * OherCcomp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult & SweepResult)
{
	...
	if (Ai)
	{
		Ai->Inventory->CurItemCount++;
		Ai->AllStop();
		Ai->IsMoveToItem = false;
		Ai->IsMoving = true;
	}
	Character->SoundManager->PlayItemGetSound();
}
